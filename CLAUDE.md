...
`examples/taylor-sparks/` and `examples/huiwen-ji/` each hold a `benchmark.yaml` and are the acceptance tests for the whole pipeline (see TASKS.md, tasks 8 and 9). Their `results/` directories are committed because they contain aggregates only; the guardrail test runs on them.
